...
export const Item = ({ id, label, onClick, icon: Icon, active, documentIcon, isSearch, level=0, onExpand, expanded,}: ItemProps) => 
    ...
    return (
        <div onClick={onClick} role="button" style={{ paddingLeft: level ? `${(level * 12 ) + 12}px`: "12px" }} className={cn("group min-h-[27px] text-sm py-1 pr-3 w-full hover:bg-primary/5 flex items-center text-muted-foreground font-medium",
            ...
            {isSearch && (
                <kbd className="ml-auto pointer-events-none inline-flex h-5 slecet-none items-center gap-1 rounded border bg-muted px-1. 5 font-mnono text-[10px] font-medium text-muted-foreground opacity-100">
